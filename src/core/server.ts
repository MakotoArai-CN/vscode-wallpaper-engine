...
interface WallpaperCssConfig {
    ...
    adaptiveColorsRespectTransparency: boolean;
    adaptiveNavAlpha: number;
    adaptiveTopAlpha: number;
}

interface WallpaperGeneralConfig {
    audioSource: 'off' | 'simulate' | 'mic' | 'system';
...
export class WallpaperServer {
    ...
    private cssConfig: WallpaperCssConfig = {
        ...
        adaptiveColorsRespectTransparency: true,
        adaptiveNavAlpha: 1,
        adaptiveTopAlpha: 1
    };
    ...
    public updateCssConfig(config: Partial<WallpaperCssConfig>) {
        this.cssConfig = { ...this.cssConfig, ...config };
    }

    public updateGeneralConfig(config: Partial<WallpaperGeneralConfig>) {
    ...
    public async start(rootPath: string, port: number, entryFile?: string, location?: string, silent = false) {
        ...
        this.server = http.createServer((req, res) => {
            ...
            if (reqUrl === '/config') {
                ...
                res.end(JSON.stringify({
                    ...this.cssConfig,
                    audioSource: this.generalConfig.audioSource,
                    interaction: this.generalConfig.interactionEnabled
                }));
                return;
            }
...
<html>
<head>
    <style>body, html { margin: 0; padding: 0; overflow: hidden; background: black; width: 100%; height: 100%; } video { width: 100%; height: 100%; object-fit: contain; }</style>
</head>
<body>
...
<html>
<head>
    <style>body, html { margin: 0; padding: 0; overflow: hidden; background: black; width: 100%; height: 100%; } img { width: 100%; height: 100%; object-fit: contain; }</style>
</head>
<body>
