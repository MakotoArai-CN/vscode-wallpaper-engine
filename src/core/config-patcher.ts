...
export const TRANSPARENT_COLOR_KEYS = [
    ...
    // Keep page-specific surfaces out of transparency rules; they need stable glass/solid backgrounds.
];

const GLASS_MANAGED_COLOR_KEYS = [
    "menu.background",
...
const GLASS_COLOR_CUSTOMIZATION_KEYS = [
    "menu.background",
...
const MANAGED_TRANSPARENCY_COLOR_KEYS = [
    ...TRANSPARENT_COLOR_KEYS,
    ...GLASS_MANAGED_COLOR_KEYS,
    ...LEGACY_PAGE_COLOR_KEYS,
    ...GLASS_COLOR_CUSTOMIZATION_KEYS
];
...
function isTransparentColor(value: unknown): boolean {
    ...
    const hex = normalized.match(/^#[0-9A-Fa-f]{8}$/);
    if (!hex) {
        return false;
    }
...
function withAlpha(hex: string, opacity: number): string {
    const alpha = Math.round(Math.min(1, Math.max(0, opacity)) * 255).toString(16).padStart(2, '0').toUpperCase();
    return `${hex}${alpha}`;
}

function applyMenuGlassColors(customizations: Record<string, unknown>) {
    const config = vscode.workspace.getConfiguration('vscode-wallpaper-engine');
    const glassEnabled = config.get<boolean>('glassEnabled') ?? true;
    if (!glassEnabled) {
        for (const key of GLASS_COLOR_CUSTOMIZATION_KEYS) {
            delete customizations[key];
        }
        return;
    }

    const tint = normalizeHexColor(config.get<string>('glassTint'), '#111827');
    const opacity = config.get<number>('glassOpacity') ?? 0.52;
    const menuOpacity = Math.min(0.74, Math.max(0.48, opacity + 0.12));

    customizations["menu.background"] = withAlpha(tint, menuOpacity);
    customizations["menu.selectionBackground"] = withAlpha('#FFFFFF', 0.18);
    customizations["menu.border"] = withAlpha('#FFFFFF', 0.26);
    customizations["menubar.selectionBackground"] = withAlpha(tint, Math.min(0.7, menuOpacity + 0.04));
    customizations["menubar.selectionBorder"] = withAlpha('#FFFFFF', 0.28);
}
...
export async function applyTransparencyPatch(target: vscode.ConfigurationTarget = vscode.ConfigurationTarget.Global) {
    ...
    // Floating menus, Quick Pick, hovers, dropdowns, and notifications are handled by the glass CSS.
    // Remove old transparent color overrides so they do not flatten the glass surfaces into plain transparency.
    for (const key of GLASS_MANAGED_COLOR_KEYS) {
        if (rules[key] !== undefined || isTransparentColor(newCustomizations[key])) {
            delete newCustomizations[key];
        }
    }

    for (const key of LEGACY_PAGE_COLOR_KEYS) {
        if (rules[key] !== undefined || isTransparentColor(newCustomizations[key])) {
            delete newCustomizations[key];
        }
    }

    applyMenuGlassColors(newCustomizations);
...
export async function removeTransparencyPatch(target: vscode.ConfigurationTarget = vscode.ConfigurationTarget.Global) {
    ...
    for (const key of MANAGED_TRANSPARENCY_COLOR_KEYS) {
