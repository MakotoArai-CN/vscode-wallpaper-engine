...
- 适配 VS Code `1.133.0` 的 workbench 目录，并加入最低版本与最新稳定版双版本测试。
- 为 Agent Sessions、Agent Host Chat、Chat 编辑器、Chat 输入区和辅助栏增加透明玻璃样式。
- 新增壁纸自适应配色：按饱和度与有效亮度加权提取主色，为左侧导航、辅助栏和顶部菜单生成可读的半透明色层。
- 设置面板新增自适应配色开关与强度控制；图片和视频直接采样，Web 壁纸优先采样项目预览图。
- 新增“尊重透明度”开关：开启后自适应配色的着色透明度会与对应区域（`sideBar.background`、`titleBar.activeBackground`）的透明度规则叠加相乘，而不是覆盖用户的透明度设置。

### 调整

- 保留 VS Code `1.106.1` 最低版本声明，同时持续对最新稳定版运行兼容性门禁。
